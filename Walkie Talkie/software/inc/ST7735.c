...
//        size      number of pixels per character pixel (e.g. size==2 prints each pixel of font as 2x2 square)
// Output: none
void ST7735_DrawCharH(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size){
  uint8_t line; // horizontal column of pixels of character in font
  int32_t i, j;
  if((x >= _width)            || // Clip right
     (y >= _height)           || // Clip bottom
     ((x + 6 * size - 1) < 0) || // Clip left
  ...
  for (i=0; i<6; i++ ) {
    ...
    for (j = 0; j<8; j++) {
      if (line & 0x1) {
		int xH = y+j;
		int yH = (_width)-(x+i-31);
		ST7735_DrawPixel(xH, yH, textColor);
      }
      line >>= 1;
    }
  }
...
void ST7735_OutCharH(char ch){
  ...
  ST7735_DrawCharH(StX*6,StY*10,ch,ST7735_YELLOW,ST7735_BLACK, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharH(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1);
  }
  return;
}
...
// inputs: ptr  pointer to NULL-terminated ASCII string
// outputs: none
void ST7735_OutStringH(char *ptr){
  while(*ptr){
    ST7735_OutCharH(*ptr);
    ptr = ptr + 1;
  }
 ...
 * @param bufX Array of X coordinate data.
 * @param bufY Array of Y coordinate data.
