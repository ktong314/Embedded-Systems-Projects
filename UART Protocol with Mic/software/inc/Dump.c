...
uint16_t DumpDataBuf[DUMPBUFSIZE];
uint32_t DumpNum;
...
void DumpInit(void){
	Timer1_Init();
	DumpNum = 0;
	previous_time = 0xFFFFFFFF;
}

void DumpCapture(uint16_t data){
    /* TODO (EE445L Lab 2): complete this. */
    // Use TIMER1_TAR_R as current time
	if(DumpNum < DUMPBUFSIZE){
		DumpTimeBuffer[DumpNum] = previous_time - TIMER1_TAR_R;
		previous_time = TIMER1_TAR_R;
